...
<style>
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  #stage{position:fixed;inset:0;z-index:2;display:flex;align-items:center;justify-content:center;
    padding:clamp(20px,4.5vh,54px) clamp(20px,5vw,80px);transition:opacity .22s ease,transform .22s ease}
  ...
  .dept.active::after{content:'示範';position:absolute;top:-9px;right:-9px;background:var(--lemon);color:#0a0e2e;
    font-size:11px;font-weight:900;padding:2px 7px;border-radius:7px;letter-spacing:.06em}
  .dept.soon{opacity:.6}
  /* Part 轉場 */
  ...
  .mpanel{position:relative;width:100%;max-width:760px;max-height:86vh;overflow:auto;background:rgba(16,18,52,.96);
  ...
  .mchap>span{display:block;color:var(--lemon-soft);font-weight:900;font-size:13px;letter-spacing:.1em;
    margin:16px 0 8px}
...
<body>
  ...
  <div class="overlay" id="menu"><div class="mpanel"><button class="mclose" id="menuClose">×</button>
    <h3>章節導覽（點頁碼直接跳）</h3><div id="menuList"></div></div></div>
...
const QEMO=[R.heart,R.thumb,R.smile,R.clap];                 /* 題目選項：❤️👍😄👏 */
...
const ACTIVE='電機營';        /* 目前只有電機營有內容 */
const FLOW_START=1;           /* 電機營流程從第 1 個場景開始 */
...
/* ===== 電機系 · 科系所學 ===== */
{ch:'電機系科系所學',m:'Part 1：科系所學',type:'part',no:1,label:'電機系科系所學'},

{ch:'電機系科系所學',m:'測驗 · 什麼是晶片',type:'quiz',kicker:'科系所學 · 第 1 題',art:'chip',
 q:'下面哪一個，最接近「晶片（IC）」的意思？',
 opts:['用來存照片、影片的記憶卡','把成千上萬個電晶體，微縮整合在一小塊半導體上的電路','一片很薄、會發光的玻璃面板','一種特別耐用的金屬電線'],ans:1,
 explain:'晶片就像一座超迷你城市，把上萬個微型開關（電晶體）擠進指甲大小的矽片裡——手機、電腦能思考，全靠它。'},
{ch:'電機系科系所學',m:'測驗 · 電路學',type:'quiz',kicker:'科系所學 · 第 2 題',art:'circuit',
 q:'電機系最基礎的「電路學」，在研究什麼？',
 opts:['電流、電壓在電阻、電容等元件裡怎麼流動、怎麼分配','怎麼把電線繞得最整齊漂亮','怎麼用程式畫出 3D 動畫','怎麼計算一棟大樓要用多少水泥'],ans:0,
 explain:'電路學是電機的「九九乘法」——搞懂電怎麼跑，之後不管做晶片、通訊還是機器人，都從這裡長出來。'},
{ch:'電機系科系所學',m:'測驗 · 通訊／Wi-Fi',type:'quiz',kicker:'科系所學 · 第 3 題',art:'antenna',
 q:'你每天用的 Wi-Fi、手機通訊，靠電機系的哪個本領？',
 opts:['把資料變成電磁波，在空中傳遞（訊號與通訊）','用很長的水管把聲音送過去','靠化學反應把訊息「燒」進空氣裡','用齒輪、皮帶把訊號一格一格轉'],ans:0,
 explain:'看不見的訊號其實一直在飛！電機系把資料編碼成電磁波送出去、再精準接回來，這就是通訊的魔法。'},

{ch:'電機系科系所學',m:'過場 · 進入課程探索',type:'cover',art:'rocket',
 title:'準備好了嗎？',sub:'好，接下來就請同學們進入到科系所學的課程內容來進行探索吧！'},

{ch:'電機系科系所學',m:'Part 2：科系所學',type:'part',no:2,label:'電機系科系所學'},

{ch:'電機系科系所學',m:'複習 · 電機系全景',type:'demo',kicker:'快速複習 · 電機系到底在幹嘛',art:'planet',
 title:'電機系，其實是一整個<span class="hl">宇宙</span>',cards:[
   {i:'energy',h:'電力與能源',p:'發電、輸電、馬達、電動車的「動力心臟」'},
   {i:'chip',h:'電子與半導體',p:'設計晶片、電路板，科技產品的大腦'},
   {i:'antenna',h:'通訊與訊號',p:'Wi-Fi、5G、手機，把資訊變電波傳遞'},
   {i:'gear',h:'控制與機器人',p:'讓機器、無人機、手臂乖乖聽話地動'},
   {i:'cpu',h:'計算機與軟硬體',p:'寫程式＋懂硬體，把兩邊整合起來'},
   {i:'telescope',h:'光電與感測',p:'螢幕、光纖、感測器，把光和訊號互換'}]},
{ch:'電機系科系所學',m:'測驗 · 同學的樣子',type:'quiz',kicker:'認識電機系 · 第 4 題',art:'astronaut',
 q:'讀電機系的同學，通常是什麼樣的人？',
 opts:['對「東西為什麼會動」很好奇，喜歡拆開來看、動手試','看到數字就想逃','只想背標準答案、不喜歡問為什麼','對科技沒興趣、也不想碰電腦'],ans:0,
 explain:'好奇＋愛動手，是電機人的共同點。不必一開始就很厲害，重點是想搞懂「它怎麼運作的？」'},
{ch:'電機系科系所學',m:'測驗 · 需要的特質',type:'quiz',kicker:'認識電機系 · 第 5 題',art:'lightbulb',
 q:'想念電機系，哪些特質會很有幫助？',
 opts:['還不錯的數學邏輯＋願意動手、不怕失敗的耐心','一定要天生是天才，普通人念不來','只要會畫畫就好，完全用不到數學','體力好、跑得快就夠了'],ans:0,
 explain:'數學是工具、耐心是燃料。願意一次次試、從失敗裡找答案的人，在電機系會走得很穩。'},

{ch:'電機系科系所學',m:'影片 · 第一部影片',type:'cover',art:'film',
 title:'影片時間',sub:'那接下來，就讓我們去看一下<span class="hl">第一部影片</span>吧！'},

{ch:'電機系科系所學',m:'Part 3：科系所學',type:'part',no:3,label:'電機系科系所學'},

{ch:'電機系科系所學',m:'反思法 · RAP',type:'frame',kicker:'看完影片，停下來想一想',art:'reflect',
 title:'用 <span class="hl">RAP</span> 來反思',steps:[
   {b:'R',c:'var(--m-blue)',h:'Review｜回顧',p:'回想剛剛探索的——電機系在學什麼？是什麼樣的人在念？'},
   {b:'A',c:'var(--m-green)',h:'Ask｜自問',p:'問問自己：這些內容，我喜歡嗎？有沒有讓我眼睛發亮的地方？'},
   {b:'P',c:'var(--m-yellow)',h:'Present｜表達',p:'把感覺說出來——等一下用表情符號評分，加上你的想法，讓我們知道你喜歡的程度。'}]},

{ch:'電機系科系所學',m:'喜好 · 學到的內容',type:'score',kicker:'喜好探索 · 1／3',art:'star',
 q:'這科系<span class="hl">學到的內容</span>，你喜歡嗎？'},
{ch:'電機系科系所學',m:'喜好 · 學習路線',type:'score',kicker:'喜好探索 · 2／3',art:'path',
 q:'大學四年要走的<span class="hl">學習路線</span>，你喜歡嗎？'},
{ch:'電機系科系所學',m:'喜好 · 學習方式',type:'score',kicker:'喜好探索 · 3／3',art:'hand',
 q:'這種「<span class="hl">動手探索、解決問題</span>」的學習方式，適合你嗎？'},

{ch:'電機系科系所學',m:'過場 · 到 Notion 撰寫',type:'cover',art:'notion',
 title:'換你出手',sub:'如果沒有問題的話，<span class="hl">讓我們到 Notion 講義去撰寫吧！</span>'},

{ch:'電機系科系所學',m:'Part 4：科系所學',type:'part',no:4,label:'電機系科系所學'},

{ch:'電機系科系所學',m:'方法 · SMART 總覽',type:'frame',kicker:'學習歷程怎麼寫',art:'notion',
 title:'用 <span class="hl">SMART</span> 五步，不卡關',steps:[
   {b:'S',c:'var(--m-blue)',h:'Summarize｜摘要',p:'這份檔案想呈現的重點，先寫在最前面。'},
   {b:'M',c:'var(--m-green)',h:'Mention｜說明',p:'說明你參加的動機，以及這個課程／活動到底在做什麼。'},
   {b:'A',c:'var(--m-yellow)',h:'Action｜行動',p:'記錄你做了哪些具體的事、採取了哪些行動。'},
   {b:'R',c:'var(--m-red)',h:'Review｜反思',p:'反思喜不喜歡、原因，以及接下來想做什麼。'},
   {b:'T',c:'var(--m-blue)',h:'Template｜模板',p:'選一個模板輸出——Canva、Notion 或其他方式都可以。'}]},
{ch:'電機系科系所學',m:'Notion · Action 行動',type:'frame',kicker:'Notion 撰寫 ①',art:'hand',
 title:'<b style="color:var(--lemon)">A</b>　Action 你做了什麼',steps:[
   {b:'1',c:'var(--m-yellow)',h:'把「實際做過的事」寫下來',p:'上了哪堂課、玩了哪些測驗與遊戲、問了學長姐什麼。'},
   {b:'2',c:'var(--m-green)',h:'越具體越好',p:'什麼時間、做了什麼、怎麼做的——細節越清楚越有說服力。'},
   {b:'3',c:'var(--m-blue)',h:'舉個例子',p:'「我完成了電機系的 5 題探索測驗，並在喜好評分按了 ❤️。」'}]},
{ch:'電機系科系所學',m:'Notion · Review 反思',type:'frame',kicker:'Notion 撰寫 ②',art:'reflect',
 title:'<b style="color:var(--lemon)">R</b>　Review 你的反思',steps:[
   {b:'1',c:'var(--m-red)',h:'我喜歡這個科系嗎？',p:'為什麼喜歡／不喜歡？試著說出原因。'},
   {b:'2',c:'var(--m-yellow)',h:'哪裡讓我有感覺？',p:'哪個部分讓我眼睛發亮、想再多看一點？'},
   {b:'3',c:'var(--m-green)',h:'寫下「下一步」',p:'我想再做什麼？查資料、問人、找相關體驗都算。'}]},

/* ===== 電機系 · 未來發展 ===== */
{ch:'電機系未來發展',m:'Part 1：未來發展',type:'part',no:1,label:'電機系未來發展'},

{ch:'電機系未來發展',m:'測驗 · 畢業出路',type:'quiz',kicker:'未來發展 · 第 1 題',art:'rise',
 q:'電機系畢業後，最主要的出路大多落在哪裡？',
 opts:['半導體與科技業：晶片設計、電子、通訊、電力、軟硬體整合','只有電力公司可以去','主要是修理手機和電腦','大多去開餐廳'],ans:0,
 explain:'電機是科技業的地基。從手機晶片到電動車、從 5G 到資料中心，背後都需要大量電機人。'},
{ch:'電機系未來發展',m:'測驗 · IC 設計工程師',type:'quiz',kicker:'未來發展 · 第 2 題',art:'cpu',
 q:'半導體公司裡的「IC 設計工程師」，主要在做什麼？',
 opts:['在工廠裡負責搬晶圓','設計晶片裡的電路，決定晶片怎麼運作、能做哪些事','幫晶片拍宣傳照','負責把晶片賣給客人'],ans:1,
 explain:'IC 設計像在指甲大的空間裡「畫城市規劃」——決定電路怎麼走，晶片才會聰明又省電。'},
{ch:'電機系未來發展',m:'測驗 · 跨領域',type:'quiz',kicker:'未來發展 · 第 3 題',art:'path',
 q:'電機的底子（電路、訊號、寫程式）除了科技業，還能跨進哪些領域？',
 opts:['什麼都跨不了，只能待在電子廠','醫療電子、電動車、AI、機器人、太空與綠能等很多領域','只能完全轉行，學的都用不到','只能做文書工作'],ans:1,
 explain:'電機是「萬用底盤」。哪裡有電、有訊號、有運算，哪裡就用得上——選擇權其實很大。'},

{ch:'電機系未來發展',m:'過場 · 進入未來發展',type:'cover',art:'rocket',
 title:'下一站',sub:'好，接下來換我們看看，電機系的「<span class="hl">未來發展</span>」長什麼樣子！'},

{ch:'電機系未來發展',m:'Part 2：未來發展',type:'part',no:2,label:'電機系未來發展'},

{ch:'電機系未來發展',m:'複習 · 產業地圖',type:'demo',kicker:'快速複習 · 畢業後能走的路',art:'rise',
 title:'電機人的<span class="hl">產業地圖</span>',four:true,cards:[
   {i:'chip',h:'半導體／IC 設計',p:'台積電、聯發科…台灣的護國群山'},
   {i:'car',h:'電動車與車用電子',p:'馬達、電池、車載晶片與系統'},
   {i:'brain',h:'AI 與資料中心',p:'AI 晶片、伺服器、運算基礎建設'},
   {i:'antenna',h:'通訊與 5G',p:'網路、基地台、衛星通訊'},
   {i:'energy',h:'綠能與電力',p:'太陽能、風電、儲能與電網'},
   {i:'pulse',h:'醫療電子與穿戴',p:'感測器、影像、健康監測裝置'},
   {i:'gear',h:'機器人與自動化',p:'智慧製造、手臂、無人載具'},
   {i:'rocket',h:'太空與國防',p:'衛星、雷達、航太電子系統'}]},
{ch:'電機系未來發展',m:'測驗 · 半導體為何需要電機',type:'quiz',kicker:'認識未來發展 · 第 4 題',art:'chip',
 q:'為什麼半導體（像台積電那類公司）特別需要電機系的人？',
 opts:['因為晶片從設計到製造，大量用到電路、訊號、材料與製程知識','因為他們需要很多人掃地','因為電機系的人都很會煮飯','因為這跟電一點關係都沒有'],ans:0,
 explain:'一顆晶片要經過設計、製造、測試一路關卡，每一關都踩在電機的核心知識上——所以特別搶電機人才。'},
{ch:'電機系未來發展',m:'測驗 · 未來趨勢',type:'quiz',kicker:'認識未來發展 · 第 5 題',art:'brain',
 q:'下面哪一個，是未來幾年很被看好、和電機高度相關的趨勢？',
 opts:['AI 晶片、電動車、再生能源、機器人','這些其實都和電機無關','電機產業即將消失','未來不會再用到晶片'],ans:0,
 explain:'AI、電動車、綠能、機器人——這幾條大浪潮，核心全都離不開電機，未來十年的舞台很大。'},

{ch:'電機系未來發展',m:'Part 3：未來發展',type:'part',no:3,label:'電機系未來發展'},

{ch:'電機系未來發展',m:'喜好 · 未來出路',type:'score',kicker:'喜好探索 · 1／3',art:'rise',
 q:'這些未來出路（設計晶片、研發新科技…），你<span class="hl">嚮往</span>嗎？'},
{ch:'電機系未來發展',m:'喜好 · 工作型態',type:'score',kicker:'喜好探索 · 2／3',art:'gear',
 q:'這種「動手研發、解決問題」的<span class="hl">工作型態</span>，適合你嗎？'},
{ch:'電機系未來發展',m:'喜好 · 願意投入',type:'score',kicker:'喜好探索 · 3／3',art:'star',
 q:'為了走到這些未來，你<span class="hl">願意投入</span>時間去學嗎？'},

{ch:'電機系未來發展',m:'Part 4：未來發展',type:'part',no:4,label:'電機系未來發展'},

{ch:'電機系未來發展',m:'過場 · 到 Notion 撰寫',type:'cover',art:'notion',
 title:'再寫一段',sub:'沒問題的話，<span class="hl">讓我們到 Notion 講義去撰寫吧！</span>把「未來發展」這段也寫下來。'},
{ch:'電機系未來發展',m:'Notion · Action 行動',type:'frame',kicker:'Notion 撰寫 ①',art:'hand',
 title:'<b style="color:var(--lemon)">A</b>　Action 你做了什麼',steps:[
   {b:'1',c:'var(--m-yellow)',h:'把這段「實際做過的事」寫下來',p:'看了哪些未來出路、玩了哪些題目、有什麼新發現。'},
   {b:'2',c:'var(--m-green)',h:'越具體越好',p:'你最有印象的產業是哪個？為什麼？把過程寫清楚。'},
   {b:'3',c:'var(--m-blue)',h:'舉個例子',p:'「我了解了 IC 設計工程師在做什麼，並對 AI 晶片特別有興趣。」'}]},
{ch:'電機系未來發展',m:'Notion · Review 反思',type:'frame',kicker:'Notion 撰寫 ②',art:'reflect',
 title:'<b style="color:var(--lemon)">R</b>　Review 你的反思',steps:[
   {b:'1',c:'var(--m-red)',h:'這些未來，我嚮往嗎？',p:'哪條路讓我心動？為什麼？'},
   {b:'2',c:'var(--m-yellow)',h:'我和它合得來嗎？',p:'這種工作型態、需要的能力，符合我的樣子嗎？'},
   {b:'3',c:'var(--m-green)',h:'寫下「下一步」',p:'想更靠近的話，我接下來可以先做什麼？'}]},

/* ===== 收尾 ===== */
{ch:'收尾',m:'收尾 · 學習總結',type:'cover',art:'flag',
 title:'今天的探索，先到這裡',sub:'科系所學 × 未來發展，你已經摸到<span class="hl">電機系的輪廓</span>了。接下來，換個方式來驗收——'},
{ch:'收尾',m:'競賽 · Kahoot',type:'cover',art:'trophy',
 title:'來玩 <span class="hl">Kahoot</span>！',sub:'準備好了嗎？我們來一場小競賽，看看你記住多少、答對多少！打開 Kahoot、輸入 PIN，開始囉～'},
{ch:'收尾',m:'午餐前 · 問學長姐 Slido',type:'frame',kicker:'吃飯前，先問問學長姐',art:'question',
 title:'到 <span class="hl">Slido</span>，用 USB 三個方向問',steps:[
   {b:'U',c:'var(--m-blue)',h:'Usually｜平常',p:'學長姐平常都在做什麼？課堂、實驗、社團，日常長什麼樣？'},
   {b:'S',c:'var(--m-green)',h:'Situation｜實況',p:'科系裡真實的狀況——和老師怎麼互動、實際在研究什麼、念起來辛不辛苦？'},
   {b:'B',c:'var(--m-yellow)',h:'Be Back｜重來',p:'如果回到高中，你會給當時的自己、或給我們什麼建議？'}],
 note:'其他想問的也都可以——直接把問題打在 Slido 上！（此處可放 Slido 畫面）'},
{ch:'收尾',m:'午餐 · 1:30 回來',type:'cover',art:'lunch',
 title:'午餐時間到！',sub:'記得先去吃飯、補充能量～ 下午 <span class="hl">1:30 準時回來</span>，別遲到喔！'},
];
...
function homeHTML(){
  const cols=DEPTS.map(col=>`<div class="dcol"><div class="dhead">${col.g}</div>${
    col.items.map(name=>{const on=name===ACTIVE;
  ...
  return `<div class="scene">
    ...
    <div class="sub" style="font-size:clamp(15px,1.7vw,20px)">主持人：點選下方營隊進入課程（目前以 <b style="color:var(--lemon)">電機營</b> 示範，其餘陸續上線）</div>
    <div class="home-grid">${cols}</div>
  </div>`;
}
function partHTML(s){
  const dots=[1,2,3,4].map(n=>`<span class="pdot${n<=s.no?' on':''}"></span>`).join('');
  return `<div class="scene">
    <div class="pe">PART</div>
...
function quizHTML(s){
  ...
  return `<div class="scene">
    ...
    <div class="explain" data-seq="1">${s.explain}</div></div>`;
}
function scoreHTML(s){
  const cards=SCORE.map(o=>`<div class="scard ${o.c}"><div class="si">${o.i}</div><b>${o.s}</b><span>${o.t}</span></div>`).join('');
  return `<div class="scene">
    <div class="kicker">${s.kicker}</div>${artHTML(s.art)}
    <div class="q">${s.q}</div>
...
function frameHTML(s){
  ...
  const note=s.note?`<div class="slido-note" data-seq="${s.steps.length+1}">${s.note}</div>`:'';
  return `<div class="scene"><div class="kicker">${s.kicker}</div>${artHTML(s.art)}
    <div class="title">${s.title}</div><div class="steps">${steps}</div>${note}</div>`;
}
function coverHTML(s){
  return `<div class="scene">${s.kicker?`<div class="kicker">${s.kicker}</div>`:''}${artHTML(s.art)}
    <div class="title">${s.title}</div>${s.sub?`<div class="sub" data-seq="1">${s.sub}</div>`:''}</div>`;
}
function build(s){
  switch(s.type){
...
function render(){
  ...
  if(s.type==='home') stage.querySelectorAll('.dept').forEach(b=>b.onclick=e=>{e.stopPropagation();pickDept(b.dataset.dept);});
  const seqs=[...stage.querySelectorAll('[data-seq]')].map(e=>+e.dataset.seq);
  maxStep=seqs.length?Math.max(...seqs):0;step=0;applyStep();
  document.getElementById('chapter').textContent=s.type==='home'?'選擇營隊':s.ch;
  document.getElementById('progress').textContent=s.type==='home'?'主頁':`${idx} / ${SCENES.length-1}`;
}
function applyStep(){
...
function next(){if(step<maxStep){step++;applyStep();return;}if(idx<SCENES.length-1)goto(idx+1);}
...
function pickDept(name){if(name===ACTIVE){goto(FLOW_START);}else{showSoon(name);}}
...
(function(){
  let html='',cur='';
  SCENES.forEach((s,i)=>{
    if(s.ch!==cur){cur=s.ch;html+=`<div class="mchap"><span>${cur}</span>`;}
    const pg=i===0?'主頁':`P${i}`;
    html+=`<button class="mitem" data-go="${i}"><span class="pg">${pg}</span><span>${s.m}</span></button>`;
  });
  document.getElementById('menuList').innerHTML=html;
  menu.querySelectorAll('.mitem').forEach(b=>b.onclick=()=>{closeOverlays();goto(+b.dataset.go);});
})();
...
function showSoon(name){
  ...
  document.getElementById('soonMsg').textContent='這個營隊的課程我們之後再一起完成 🚧　今天先用「電機營」示範完整流程。';
